...
map_data_world_bank = (
    read_csv("data/map_data_world_bank.csv").drop(["lng", "lat"], axis=1).merge(points_from_polygons, on="Code")
)
map_data_oecd = read_csv("data/map_data_oecd.csv").drop(["lng", "lat"], axis=1).merge(points_from_polygons, on="Code")
